...
import { ButtonMenuItem, Flex, Button, Text } from '@apeswapfinance/uikit'
...
export default function RemoveLiquidity({
  ...
  function modalHeader() {
    return (
      <AutoColumn gap="md" style={{ marginTop: '20px' }}>
        ...
        <Italic fontSize={12} color={theme.colors.text} textAlign="left" padding="12px 0 0 0">
  ...
  function modalBottom() {
    return (
      <>
        <RowBetween>
          <Text color="text">{`APE-LP ${currencyA?.symbol}/${currencyB?.symbol}`} Burned</Text>
          <RowFixed>
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        history.push(`/trade/remove/${currencyId(currency)}/${currencyIdA}`)
      } else {
        history.push(`/trade/remove/${currencyId(currency)}/${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        history.push(`/trade/remove/${currencyIdB}/${currencyId(currency)}`)
      } else {
        history.push(`/trade/remove/${currencyIdA}/${currencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, history]
  )
                  ...
                  {!showDetailed && (
                    <>
                      ...
                      <Flex justifyContent="space-around">
                        <ButtonMenuItem
                          isActive={innerLiquidityPercentage === 25}
                          size="sm"
                        ...
                        <ButtonMenuItem
                          isActive={innerLiquidityPercentage === 50}
                          size="sm"
                        ...
                        <ButtonMenuItem
                          isActive={innerLiquidityPercentage === 75}
                          size="sm"
                        ...
                        <ButtonMenuItem
                          isActive={innerLiquidityPercentage === 100}
                          size="sm"
                    ...
                    <AutoColumn gap="10px">
                      ...
                      {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                        <RowBetween style={{ justifyContent: 'flex-end' }}>
                          {oneCurrencyIsETH ? (
                            <StyledInternalLink
                              to={`/trade/remove/${currencyA === ETHER ? WETH[chainId].address : currencyIdA}/${
