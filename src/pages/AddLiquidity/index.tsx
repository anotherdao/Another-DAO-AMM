...
import { CardBody, AddIcon, Text as UIKitText, Button } from '@apeswapfinance/uikit'
  ...
  const handleCurrencyASelect = useCallback(
    (currA: Currency) => {
      ...
      if (newCurrencyIdA === currencyIdB) {
        history.push(`/trade/add/${currencyIdB}/${currencyIdA}`)
      } else {
        history.push(`/trade/add/${newCurrencyIdA}/${currencyIdB}`)
      }
    },
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        if (currencyIdB) {
          history.push(`/trade/add/${currencyIdB}/${newCurrencyIdB}`)
        } else {
          history.push(`/trade/add/${newCurrencyIdB}`)
        }
      } else {
        history.push(`/trade/add/${currencyIdA || 'ETH'}/${newCurrencyIdB}`)
      }
    },
                ...
                <ColumnCenter>
                  <Pane>
                    <AutoColumn gap="12px">
                      ...
                      <UIKitText>{t('The ratio of tokens you add will set the price of this pool.')}</UIKitText>
                      <UIKitText>{t('Once you are happy with the rate click supply to review.')}</UIKitText>
                      <UIKitText>
                        {t('Thinking about locking it up? Do it at ')}
                        <StyledLink href="https://cryptexlock.me/r/0rALjfjXvoTKZAPCYE6PMAZNBXvn1" color="yellow">
                          CryptEx
                        </StyledLink>
                      </UIKitText>
                    </AutoColumn>
